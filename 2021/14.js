...
const rules = data.slice(2).filter(e => e).map(e => e.split(" -> "));

function part1(template, rules) {
  for (let step = 0; step < 10; step++) {
    for (let i = 0; i < template.length - 1; i++) {
      const rule = rules.find(e => e[0][0] === template[i] && e[0][1] === template[i + 1]);
      if (rule) {
  ...
  return Math.max(...Object.values(occurences)) - Math.min(...Object.values(occurences));
}
...
function part2(template, rules) {
  ...
  for (let i = 0; i < template.length - 1; i++) {
    if (!pairs[template[i] + template[i + 1]]) pairs[template[i] + template[i + 1]] = 0;
  ...
  return Math.ceil(Math.max(...Object.values(occurences)) - Math.min(...Object.values(occurences)));
}
